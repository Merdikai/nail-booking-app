...
export default function Admin() {
  ...
  const fetchBookings = async () => {
    ...
    const { data, error } = await supabase
      .from("bookings")
      .select(`*, designs (id, name, image_url, price)`)
  ...
  const handleAddDesign = async (e: React.FormEvent) => {
    ...
    if (uploadMethod === "file" && !selectedFile) { alert("Please select an image file"); setLoading(false); return; }
    if (uploadMethod === "url" && !designImageUrl) { alert("Please enter an image URL"); setLoading(false); return; }
    if (!designName || !designBrand || !designCategory) { alert("Please fill in all required fields"); setLoading(false); return; }
    ...
    try {
      ...
      const { error } = await supabase.from("designs").insert([
        {
          ...
          price: parseFloat(designPrice) || 0,
      ...
      setUploadSuccess("Design added successfully! 🎉");
      setDesignName(""); setDesignImageUrl(""); setDesignDescription("");
      setDesignBrand(""); setDesignCategory(""); setDesignPrice("");
  ...
  const handleDeleteDesign = async (id: number, imageUrl: string) => {
    ...
    if (imageUrl.includes("supabase")) {
      try {
        const urlParts = imageUrl.split("/design-images/");
        if (urlParts.length > 1) await supabase.storage.from("design-images").remove([urlParts[1]]);
      } catch (err) { console.log("Could not delete from storage:", err); }
    }
          ...
          <div className="row align-items-center">
            <div className="col-md-2 text-center">
              {booking.designs ? (
                <>
                  <img
                    src={booking.designs.image_url}
                    alt={booking.designs.name}
                    className="booking-design-img"
                    onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }}
                  />
                  <div className="mt-1">
                    <span className="badge booking-design-badge">💅 {booking.designs.name}</span>
                  </div>
                </>
              ) : (
                <div className="booking-no-design">
                  <span className="text-muted small">No design</span>
                </div>
              )}
            </div>
            ...
            <div className="col-md-6">
              ...
              <div className="row">
                <div className="col-sm-6">
                  <p className="mb-1 small">📞 {booking.phone}</p>
                  <p className="mb-1 small">📅 {formatDate(booking.appointment_date)}</p>
                </div>
                <div className="col-sm-6">
                  <p className="mb-1 small">⏰ {booking.appointment_time}</p>
                  {booking.designs?.price && booking.designs.price > 0 && (
                    <p className="mb-1 small">💰 ${booking.designs.price}</p>
                  )}
                </div>
              </div>
          ...
          <div className="col-12">
            ...
            <ul className="admin-nav-tabs nav nav-tabs mb-4">
              <li className="nav-item">
                ...
                >
                  📅 Bookings
                  <span className="badge admin-badge-count ms-2">{activeBookings.length}</span>
                </button>
              </li>
              <li className="nav-item">
                <button
                  className={`nav-link ${activeTab === "designs" ? "active" : ""}`}
                  onClick={() => { setActiveTab("designs"); fetchDesigns(); }}
                >
              ...
              <li className="nav-item">
                <button
                  className={`nav-link ${activeTab === "add-design" ? "active" : ""}`}
                  onClick={() => setActiveTab("add-design")}
                >
                  ➕ Add New Design
                </button>
              </li>
            ...
            {activeTab === "bookings" && (
              <>
                <div className="admin-filter-buttons">
                  <button
                    className={`admin-filter-btn ${bookingFilter === "active" ? "active" : ""}`}
                    onClick={() => setBookingFilter("active")}
                  >
                    📋 Active
                    <span className="badge ms-2">{activeBookings.length}</span>
                  </button>
                  <button
                    className={`admin-filter-btn ${bookingFilter === "archived" ? "active" : ""}`}
                    onClick={() => setBookingFilter("archived")}
                  >
                    📁 Archived
                    <span className="badge ms-2">{archivedBookings.length}</span>
                  </button>
                </div>
                  ...
                  {loading ? (
                    ...
                    <div className="col-12">
                      <div className="admin-empty-state">
                        ...
                        <h4>{bookingFilter === "active" ? "No Active Bookings!" : "No Archived Bookings"}</h4>
                        <p className="text-muted">
                          {bookingFilter === "active"
                            ? "All caught up! New bookings will appear here."
                            : "Completed or cancelled bookings will move here."}
                        </p>
                      </div>
                    </div>
                  ) : (
              ...
              <div className="row">
                {loading ? (
                  ...
                  <div className="col-12 text-center py-5">
                    ...
                    <p className="text-muted">Start adding nail designs for your customers!</p>
                    <button className="btn btn-pink mt-2" onClick={() => setActiveTab("add-design")}>
                      ➕ Add First Design
                    </button>
                  </div>
                ) : (
                        ...
                        <img
                          ...
                          onError={(e) => {
                            (e.target as HTMLImageElement).src =
                              "https://placehold.co/400x300/f8d7da/721c24?text=Image+Not+Found";
                          }}
                        />
                        <div className="card-body">
                          ...
                          <button
                            className="btn admin-delete-btn btn-sm w-100"
                            onClick={() => handleDeleteDesign(design.id, design.image_url)}
                          >
                            🗑️ Delete
                          </button>
                        </div>
                      ...
                      <form onSubmit={handleAddDesign}>
                        <div className="mb-3">
                          ...
                          <div className="d-flex gap-2">
                            <button
                              type="button"
                              className={`btn flex-grow-1 ${uploadMethod === "file" ? "btn-pink" : "btn-outline-muted"}`}
                            ...
                            <button
                              type="button"
                              className={`btn flex-grow-1 ${uploadMethod === "url" ? "btn-pink" : "btn-outline-muted"}`}
                        ...
                        {uploadMethod === "url" && (
                          <div className="mb-3">
                            ...
                            <input type="url" className="form-control" placeholder="https://example.com/image.jpg" value={designImageUrl} onChange={(e) => setDesignImageUrl(e.target.value)} />
                          </div>
                        )}
                        ...
                        <div className="mb-3">
                          ...
                          <input type="text" className="form-control" placeholder="e.g., Summer Floral" value={designName} onChange={(e) => setDesignName(e.target.value)} required />
                        </div>
                        <div className="mb-3">
                          <label className="form-label">Description</label>
                          <textarea className="form-control" rows={2} placeholder="Describe this design..." value={designDescription} onChange={(e) => setDesignDescription(e.target.value)} />
                        </div>
                        <div className="row">
                          <div className="col-md-6 mb-3">
                            <label className="form-label">Brand *</label>
                            <input type="text" className="form-control" placeholder="e.g., OPI" value={designBrand} onChange={(e) => setDesignBrand(e.target.value)} required />
                          </div>
                          <div className="col-md-6 mb-3">
                            <label className="form-label">Category *</label>
                            <select className="form-select" value={designCategory} onChange={(e) => setDesignCategory(e.target.value)} required>
                        ...
                        <div className="mb-3">
                          ...
                          <input type="number" className="form-control" placeholder="0.00" step="0.01" min="0" value={designPrice} onChange={(e) => setDesignPrice(e.target.value)} />
                        </div>
